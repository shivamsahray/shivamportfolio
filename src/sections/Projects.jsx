...
import photo2 from "../assets/photo21.png"
import photo3 from "../assets/photo31.png"
...
export default function Projects() {
  ...
  const projects = useMemo(
    () => [
      {
        title: "Whatsy",
        link: "https://whatsy-messanger.onrender.com/",
        bgColor: "#0d4d3d",
        image: isMobile ? photo1 : img1, // use mobile or desktop image
      },
      {
        title: "Meddium",
        link: "https://meddium.netlify.app/",
        bgColor: "#3884d3",
        image: isMobile ? photo2 : img2,
      },
      {
        title: "StarOne",
        link: "https://staronepackaging.netlify.app/",
        bgColor: "#dc9317",
      ...
      <div className="sticky top-0 h-screen flex flex-col items-center justify-center ">
        ...
        <div className={`relative w-full flex-1 flex items-center justify-center ${isMobile ? "-mt-4" : ""}`}>
          {projects.map((project, idx) => (
            <div key={project.title} className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-500 ${
              ...
              <AnimatePresence mode="wait">
                {activeIndex === idx && (
                  <motion.h3 key={project.title}
                    ...
                    className={`block text-center text-[clamp(2rem,6vw,5rem)] text-white/95 sm:absolute sm:-top-20 sm:left-[-35%] lg:left-[-5%] sm:mb-0 pb-3
        ...
        <div className={`absolute ${isMobile ? "bottom-20" : "bottom-10"}`}>
          <a href={activeProject?.link} target="_blank"
            ...
            className="inline-block px-6 py-3 font-semibold rounded-lg bg-white text-black hover:bg-gray-200 transition-all" aria-label={`View ${activeProject?.title}`}
          >
            View Project</a>
        </div>
