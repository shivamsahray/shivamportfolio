...
export default function Navbar() {
  ...
  //useEffect for 3 sec navbar hide after scroll
  useEffect(() => {
    const handleScroll = () => {
      if(forceVisible){
        setVisible(true);
  ...
  return (
   ...
   <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-6 py-4 z-50 transition-transform duration-300 ${visible ? "translate-y-0" : "-translate-y-full"}`}>
    ...
    <div className="flex items-center space-x-2">
       ...
       <div className="text-2xl font-bold hidden sm:block">Shivam</div>
    </div>
    ...
    <div className="hidden lg:block">
      <a href="#contact" className="bg-gradient-to-r from-pink-500 to-blue-500 texxt-white px-5 py-2 rounded-full font-medium shadow-lg hover:opacity-90 transition-opacity duration-300">
